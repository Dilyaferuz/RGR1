		...
		<div class="flat">
			<div class="view"> <?php
				...
				if (isset ($room['areaTotal'])){
					?> <div class="home"> Общая площадь: <?=htmlspecialchars($room['areaTotal'])?> </div>
					<?php 
				}
				if (isset ($room['areaLive'])){
					?> <div class="home"> Жилая площадь: <?=htmlspecialchars($room['areaLive'])?> </div>
					<?php
				}
				if (isset ($room['areaKitchen'])){
					?> <div class="home"> Кухонная площадь: <?=htmlspecialchars($room['areaKitchen'])?> </div>
					<?php 
				} ?>
			</div>
		...
		<div class="photos"> <?php
			if (isset($room['photos'])){
				foreach ($room['photos'] as $key => $photo) {
					echo "<p><img class='photo' src=$photo><p>";
				}
			} ?>
		</div> 
		</div>	<?
